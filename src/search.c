...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = log(depth) * log(moves) / 2.1350 + 0.2319;
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = 1.4309 + 0.3339 * depth * depth;
...
void StartSearch(Board* board, uint8_t ponder) {
  ...
  // Reset instrumentation state for the new search.
...
#if defined(NEXUS_PROFILE)
  if (NexusProfileReportEnabled) {
    NexusProfileReport();
  }
#endif

#if defined(NEXUS_SUMMARY)
  if (NexusExplainEnabled) {
    NexusSummaryPrintExplain();
  }
#endif
}
      ...
      while (1) {
        ...
        if (score <= alpha) {
          ...
          if (mainThread)
            Threads.stopOnPonderHit = 0;
#if defined(NEXUS_PROFILE)
          NEXUS_PROFILE_INC(NPC_ASPIRATION_RETRIES);
#endif
#if defined(NEXUS_SUMMARY)
          NexusSummaryRecordAspirationRetry();
#endif
        } else if (score >= beta) {
...
#if defined(NEXUS_SUMMARY)
    if (mainThread) {
      NexusSummaryRecordIteration(thread->depth, thread->seldepth,
                                  (int64_t)thread->nodes,
                                  FromTo(bestMove),
                                  (int64_t)thread->rootMoves[0].nodes,
                                  thread->numRootMoves);
    }
#endif
  }
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
...
#if defined(NEXUS_PROFILE)
  NEXUS_PROFILE_INC(NPC_NEGAMAX_CALLS);
#endif
...
#if defined(NEXUS_PROFILE)
    NEXUS_PROFILE_INC(NPC_TT_SCORE_CUTS);
#endif
#if defined(NEXUS_SUMMARY)
    NexusSummaryRecordCutoff(0);
#endif
    return ttScore;
  }
  ...
  if (!isPV && !inCheck) {
    ...
    if ((ss - 1)->reduction >= 1 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 79)
      depth--;

    // Reverse Futility Pruning
    // i.e. the static eval is so far above beta we prune
    if (depth <= 10 && !ss->skip && eval < TB_WIN_BOUND && eval >= beta &&
        eval - 83 * depth + 148 * (improving && !opponentHasEasyCapture) + 25 * opponentDeclining >= beta &&
        (!hashMove || GetHistory(ss, thread, hashMove) > 10790)) {
#if defined(NEXUS_PROFILE)
      NEXUS_PROFILE_INC(NPC_RFP_CUTS);
#endif
#if defined(NEXUS_SUMMARY)
      NexusSummaryRecordCutoff(2);
#endif
      return (eval + beta) / 2;
    }
    ...
    // threats)
    if (depth >= 5 && (ss - 1)->move != NULL_MOVE && !ss->skip && !opponentHasEasyCapture && eval >= beta &&
        HasNonPawn(board, board->stm) && (ss->ply >= thread->nmpMinPly || board->stm != thread->npmColor)) {
      int R = 4 + 367 * depth / 1024 + Min(9 * (eval - beta) / 1024, 4);
...
#if defined(NEXUS_PROFILE)
          NEXUS_PROFILE_INC(NPC_NULL_MOVE_CUTS);
#endif
#if defined(NEXUS_SUMMARY)
          NexusSummaryRecordCutoff(1);
#endif
          return score;
        }
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (bestScore > -TB_WIN_BOUND) {
      ...
      if (!IsCap(move) && PromoPT(move) != QUEEN) {
        ...
        if (!killerOrCounter && lmrDepth < 4 && history < -2850 * (depth - 1)) {
          skipQuiets = 1;
          continue;
        }

        if (!inCheck && lmrDepth < 10 && eval + 66 + 44 * lmrDepth <= alpha)
          skipQuiets = 1;
    ...
    if (!isRoot && ss->ply < thread->depth * 2) {
      ...
      if (depth >= 6 && move == hashMove && ttDepth >= depth - 3 && (ttBound & BOUND_LOWER) &&
        ...
        if (score < sBeta) {
#if defined(NEXUS_PROFILE)
          NEXUS_PROFILE_INC(NPC_SINGULAR_EXTENSIONS);
#endif
          if (!isPV && score < sBeta - 43 && ss->de <= 7 && !IsCap(move)) {
            extension = 3;
            ss->de    = (ss - 1)->de + 1;
          } else if (!isPV && score < sBeta - 12 && ss->de <= 6) {
            extension = 2;
            ss->de    = (ss - 1)->de + 1;
          } else {
            extension = 1;
...
#if defined(NEXUS_PROFILE)
      NEXUS_PROFILE_INC(NPC_LMR_REDUCTIONS);
#endif
    ...
    if (score > bestScore) {
      ...
      // we're failing high
      if (alpha >= beta) {
        UpdateHistories(ss, thread, move, depth + (bestScore > beta + 75), quiets, numQuiets, captures, numCaptures);
        break;
      }
    }
  }
...
int Quiesce(int alpha, int beta, int depth, ThreadData* thread, SearchStack* ss) {
...
#if defined(NEXUS_PROFILE)
  NEXUS_PROFILE_INC(NPC_QUIESCE_CALLS);
#endif
#if defined(NEXUS_SUMMARY)
  NexusSummaryRecordQsearchNodes(1);
#endif
  ...
  if (inCheck) {
    ...
    if (eval >= beta) {
#if defined(NEXUS_PROFILE)
      NEXUS_PROFILE_INC(NPC_QS_STANDPAT_CUTS);
#endif
      return eval;
    }
