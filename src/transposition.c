...
inline TTEntry* TTProbe(uint64_t hash,
  ...
  NEXUS_PROFILE_INC(NPC_TT_PROBES);
#endif
#if defined(NEXUS_SUMMARY)
  NexusSummaryRecordTT(0);  // tentative; will increment hit below
#endif
  ...
  for (int i = 0; i < BUCKET_SIZE; i++) {
    if (bucket[i].hash == shortHash || !bucket[i].depth) {
      *hit = !!bucket[i].depth;
      ...
      if (*hit) {
#if defined(NEXUS_PROFILE)
        NEXUS_PROFILE_INC(NPC_TT_HITS);
#endif
#if defined(NEXUS_SUMMARY)
        NexusSummary.ttHits++;  // correct the tentative miss above
#endif
