...
// Third order polynomial fit of Nexus 6.0 data
const double as[4] = {0.01538655, -1.33196512, 7.43032607, 152.07701643};
const double bs[4] = {-2.66442843, 18.35463702, -38.91760538, 56.27652511};
...
void PrintUCIOptions() {
  ...
  // Nexus 6.1 instrumentation options. Always declared so UCI handshakes are
  // consistent across builds; setting them in a non-instrumented build is a
  // safe no-op (the engine will print an info-string notice on first use).
  printf("option name DebugModules type string default <empty>\n");
  printf("option name ExplainMode type check default false\n");
  printf("option name ProfileReport type check default false\n");
  printf("info string build debug=%d profile=%d summary=%d\n",
